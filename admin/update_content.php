...
<section class="video-form">
   ...
   <form action="" method="post" enctype="multipart/form-data">
      ...
      <p>actualizar estado <span>*</span></p>
      <select name="status" class="box" required>
         <option value="<?= $fecth_videos['status']; ?>" selected><?= $fecth_videos['status']; ?></option>
         <option value="active">activa</option>
         <option value="deactive">inactiva</option>
      </select>
      <p>actualizar titulo <span>*</span></p>
      <input type="text" name="title" maxlength="100" required placeholder="ingresa el titulo del video" class="box" value="<?= $fecth_videos['title']; ?>">
      <p>actualizar descripcion <span>*</span></p>
      <textarea name="description" class="box" required placeholder="escribir descripcion" maxlength="1000" cols="30" rows="10"><?= $fecth_videos['description']; ?></textarea>
      <p>actualizar playlist</p>
      <select name="playlist" class="box">
         <option value="<?= $fecth_videos['playlist_id']; ?>" selected>seleccionar playlist</option>
         <?php
         ...
         <?php
            ...
            echo '<option value="" disabled>todavia no se han creado playlist!</option>';
         }
         ?>
      </select>
      <img src="../uploaded_files/<?= $fecth_videos['thumb']; ?>" alt="">
      <p>actualizar miniatura</p>
      <input type="file" name="thumb" accept="image/*" class="box">
      <video src="../uploaded_files/<?= $fecth_videos['video']; ?>" controls></video>
      <p>actualizar video</p>
      <input type="file" name="video" accept="video/*" class="box">
      <input type="submit" value="actualizar contenido" name="update" class="btn">
      <div class="flex-btn">
         <a href="view_content.php?get_id=<?= $video_id; ?>" class="option-btn">ver contenido</a>
         <input type="submit" value="borrar contenido" name="delete_video" class="delete-btn">
      </div>
   </form>
   <?php
         }
      }else{
         echo '<p class="empty">video no encontrado! <a href="add_content.php" class="btn" style="margin-top: 1.5rem;">añadir videos</a></p>';
      }
   ?>
